...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@galyarder-framework/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@galyarder-framework/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@galyarder-framework/adapter-utils").TranscriptEntry[];
